    ...
    try:
        with engine.connect() as connection:
            ...
            columns_query = text(f"""
                ...
                WHERE table_name = :view_name
            """)
            columns_result = connection.execute(columns_query, {"view_name": view_name}).fetchall()
            # Query for sample rows
            sample_rows_query = text(f"SELECT * FROM {view_name} LIMIT :limit")
            sample_rows_result = connection.execute(sample_rows_query, {"limit": sample_row_limit}).fetchall()
            # Fetch rows
...
def get_procedure_details(engine, routine_name):
    try:
        with engine.connect() as connection:
            ...
            """)
            params_result = connection.execute(params_query, {"routine_name": routine_name}).fetchall()

            return {
                "parameters": [
                    {"name": row[0], "type": row[1], "mode": row[2]} for row in params_result
                ] if params_result else [],
            }
    except Exception as e:
...
def get_function_details(engine, routine_name):
    try:
        with engine.connect() as connection:
            ...
            """)
            result = connection.execute(details_query, {"routine_name": routine_name}).fetchone()
...
def format_table_schema_as_markdown(schema, table_name, sample_rows):
    ...
    if schema["foreign_keys"]:
        ...
        for fk in schema["foreign_keys"]:
            schema_md += (
                f"- `{fk['column']}` references `{fk['referred_table']}.{', '.join(fk['referred_columns'])}`\n"
            )
    ...
    if sample_rows:
        ...
        schema_md += "| " + " | ".join(["-" * len(key) for key in sample_rows[0].keys()]) + " |\n"
        for row in sample_rows:
            schema_md += "| " + " | ".join(str(value) for value in row.values()) + " |\n"
    return schema_md
...
def format_view_details_as_markdown(view_name, details):
    ...
    if details['sample_rows']:
        ...
        markdown += "| " + " | ".join(["-" * len(key) for key in details['sample_rows'][0].keys()]) + " |\n"
...
def main():
    ...
    for procedure in procedure_names:
        ...
        if details:
            ...
            save_markdown_file(os.environ["PROCEDURE_SCHEMA_PATH"], file_name, schema_md)
        else:
